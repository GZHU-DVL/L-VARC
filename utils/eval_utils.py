...
def get_majority_vote(predictions):
    ...
    输入 predictions 结构:
    [
      {"grid": [[1,1]], "source": "flip_0"},
      {"grid": [[1,1]], "source": "flip_0"},   <-- 来自 perm1 (同源)
      {"grid": [[1,1]], "source": "rotate_90"} <-- 来自不同源 (异源)
    ]
    """
    vote_tally = {}  # 记录加权总分
    source_tracker = {}  # 记录每个答案见过的几何视角集合 Set
    grid_lookup = {}  # 方便最后根据 Key 找回 Grid 对象

    for item in predictions:
        grid = item["grid"]
        # 如果是旧数据或者没有 source，标为 unknown
        source = item.get("source", "unknown")

        # 将 Grid 序列化作为字典的 Key
        grid_str = json.dumps(grid)
        grid_lookup[grid_str] = grid

        # 初始化
        if grid_str not in source_tracker:
            source_tracker[grid_str] = set()
            vote_tally[grid_str] = 0.0

        # 1. 记录来源
        source_tracker[grid_str].add(source)

        # 2. 基础得票 (Basic Vote)
        # 每个预测本身值 1 票 (或者你可以设小一点，依靠 Bonus 提分)
        vote_tally[grid_str] += 1.0

    # --- [核心逻辑：几何一致性奖励] ---
    for grid_str, sources in source_tracker.items():
        # unique_sources 是这个答案在多少个不同几何视角下依然成立
        # 比如 {"flip_0", "rotate_90", "original"} -> 3
        num_unique_views = len(sources)

        if num_unique_views > 1:
            # 奖励公式：指数级奖励
            # 2个视角 -> 权重 x3
            # 3个视角 -> 权重 x9
            # 4个视角 -> 权重 x27 (几乎必胜)
            # 这种设计是为了极大地奖励那些“平移/旋转/翻转”后都一致的答案
            bonus_multiplier = 3.0 ** (num_unique_views - 1)

            vote_tally[grid_str] *= bonus_multiplier
    ...
    sorted_lists = [
        {
            "prediction": grid_lookup[key],
            "votes": score,
            "unique_sources": list(source_tracker[key])  # 调试用：看看是哪些视角立功了
        }
        for key, score in sorted_votes
    ]
