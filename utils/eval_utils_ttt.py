...
def _extract_geometric_source(task_name: str) -> str:
    """
    从文件名中提取核心几何变换，忽略颜色排列和后缀。

    输入示例:
      "1ae2feb7_flip_0_augmented" -> "flip_0"
      "1ae2feb7_flip_0_perm5_augmented" -> "flip_0"
      "1ae2feb7_rotate_90_perm2_augmented" -> "rotate_90"
      "1ae2feb7_chain_rotate_90_random_translate_xy_augmented" -> "chain_rotate_90_random_translate_xy"
      "1ae2feb7" -> "original"
    """
    # 1. 去掉 _augmented 后缀
    name = task_name.replace("_augmented", "")

    # 2. 去掉 _perm{N} (例如 _perm1, _perm12)
    # 使用正则替换，将 _perm 及其后的数字去掉
    name = re.sub(r"_perm\d+", "", name)

    # 3. 去掉 Task Hash (通常在开头)
    # 假设 Hash 是由字母数字组成，后面跟着下划线
    # 如果你的 Hash 长度固定是8位 (如 1ae2feb7)，可以直接切片，或者用 split
    parts = name.split('_')

    # 简单的启发式：如果第一部分看起来像 Hash (字母+数字混合)，就去掉它
    # 或者更简单：因为我们只关心变换部分，我们可以直接忽略第一部分
    if len(parts) > 1:
        # 重新组合剩下的部分，这就是几何变换的标识
        return "_".join(parts[1:])
    else:
        # 如果切分后只剩一个部分，说明它就是原题 (original)
        return "original"
    ...
    for _ in range(attempt_nums):
        for batch in tqdm(loader):
            ...
            for idx, task_name in enumerate(batch["task_names"]):
                ...
                # --- [新增] 解析几何来源 ---
                # 这里的 task_name 就是你文件名里的 stem，例如 "1ae2feb7_flip_0_perm5_augmented"
                geo_source = _extract_geometric_source(task_name)
                ...
                task_predictions[cur_index].append({
                    "grid": predict_grid,
                    "source": geo_source  # 关键标签：例如 "flip_0"
                })
